...
build_context() {
  ...
  {
    echo "=== ACTIVE NOVEMBER READINESS PLAN — FIT ALL WORK WITHIN WEEKLY CAPACITY ==="
    cat "$PROJECT/crash-course/NOVEMBER_READINESS.md" 2>/dev/null || echo "(readiness guide missing)"
    echo
